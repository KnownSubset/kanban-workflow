...
moduleForModel('card', '', {
  needs: ['model:column', 'model:board']
  setup: ->
    Board.FIXTURES = [
      {
        id:1,
...
test("Card is a valid ember-data Model", ->
  store = this.store()
  Em.run ->  store.find('card', 1).then (card) ->
...
test("the store is able to retrieve the fixture data", ->
  store = this.store()
  Em.run ->  store.find('card', 1).then (card) ->
    new Ember.RSVP.Promise (resolve) ->
      Ember.run.later ->
        ok(card)
        fixture = Card.FIXTURES[0]
        equal(card.get('name'), fixture.name)
        equal(card.get('description'), fixture.description)
        resolve()
)
