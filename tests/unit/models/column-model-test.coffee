...
moduleForModel('column', '', {
  needs: ['model:card', 'model:board', 'model:activity-item','model:organization','model:directory','model:user-group','model:role','model:permission','model:account','model:user'],
  setup: ->
    Board.FIXTURES = [
      {
        id:1,
    ...
    Column.FIXTURES = [
        {id:1, name: 'column #1_1', kind: 'manual', createdAt: Date(), cards: [1,2,3], board: 1 },
...
test("Column is a valid ember-data Model", ->
  store = this.store()
  Em.run ->  store.find('column', 1).then (column) ->
    ok(column)
...
test("able to load the details for a column", ->
  store = this.store()
  Em.run -> store.find('column', 1).then (column) ->
    fixture = Column.FIXTURES[0]
    equal(column.get('name'), fixture.name)
    equal(column.get('kind'), fixture.kind)
    equal(column.get('createdAt'), fixture.createdAt)
)
